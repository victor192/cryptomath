  ...
  <div class="register-content">
    <ui-card class="register-content__form">
      <div class="register-content__form_title">
        ...
        <span class="help fonts__text3 colors__font_gray">{{
          $t("auth.register.form.help")
        }}</span>
      </div>
      ...
      <div class="register-content__form_fields">
        ...
        >
            ...
            <transition name="fade">
              <ui-form-alert v-if="errors.displayName">
                <span class="fonts__text2">{{
                  $t("auth.register.form.display_name.error")
                }}</span>
              </ui-form-alert>
            </transition>
            <ui-form-input
          ...
          <template slot="help">
            <span class="fonts__text4 colors__font_pale-sky">{{
              $t("auth.register.form.display_name.help")
            }}</span>
          </template>
        </ui-form-group>
        <!--  Email -->
        <ui-form-group label-for="email" help-id="email-help" class="field">
          <template slot="label">
            <span class="fonts__text-label">{{
          ...
          <template slot="help">
            <span class="fonts__text4 colors__font_pale-sky">{{
              $t("auth.register.form.email.help")
            }}</span>
          </template>
        </ui-form-group>
        <!--  Password  -->
        ...
        >
          ...
          <template slot="input">
            <transition name="fade">
              <ui-form-alert
                v-if="errors.password.error && errors.password.type"
              >
          ...
          <template slot="help">
            <span class="fonts__text4 colors__font_pale-sky">{{
              $t("auth.register.form.password.help")
            }}</span>
          </template>
        </ui-form-group>
        <!--  Captcha -->
        <ui-form-group label-for="captcha" help-id="captcha-help" class="field">
          <template slot="label">
            <span class="fonts__text-label">{{
              $t("auth.register.form.captcha.label")
            }}</span>
          </template>
          <template slot="input">
          ...
          <template slot="help">
            <span class="fonts__text4 colors__font_pale-sky">{{
              $t("auth.register.form.captcha.help")
            }}</span>
          </template>
        </ui-form-group>
        <div class="register-content__form_buttons">
          <ui-button :variant="['primary']" @click.native="submitRegister">
            {{ $t("auth.register.form.sign_up") }}
          </ui-button>
        </div>
        <div class="register-content__form_terms">
          ...
          >
            <nuxt-link
              ...
              target="_blank"
            >
              {{ $t("auth.register.form.terms.terms") }}
            </nuxt-link>
            <nuxt-link
              slot="privacy_link"
              ...
              target="_blank"
            >
              {{ $t("auth.register.form.terms.privacy") }}
            </nuxt-link>
          </i18n>
        </div>
      </div>
    </ui-card>
    <ui-card class="register-content__login" center>
      <i18n path="auth.register.login.text" tag="span" class="fonts__text2">
        <nuxt-link slot="link" to="/auth/login">
